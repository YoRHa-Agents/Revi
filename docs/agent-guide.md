...
View the current server configuration, including whether a workspace is configured.

**Response — 200 OK**

```json
{
  "workspacePath": "/home/user/.revi/workspace",
  "workspaceConfigured": true,
  "dataPath": "/home/user/.revi/data",
  "port": 8000,
  "configFile": "/home/user/.config/revi/config.toml"
}
```
...
| Field | Type | Description |
|-------|------|-------------|
| `workspacePath` | string \| null | Current workspace directory, or null if not configured |
| `workspaceConfigured` | boolean | Whether a workspace was explicitly set (CLI, config file, or API) |
| `dataPath` | string | Directory for comments, archive, and metadata |
...
Update the server configuration at runtime. Changes to `workspacePath` take effect immediately — the workspace scanner is hot-reloaded and the config is persisted to disk. No server restart is needed.
...
Returns the updated `ConfigResponse` (same schema as GET).

**Errors**

| Code | Condition |
|------|-----------|
| 400 | Cannot create workspace directory |

---

## camelCase Note

All JSON responses use **camelCase** field names. When sending request bodies, both camelCase and snake_case are accepted. Examples:
